...
function formatTimelineDateRange(startDateStr: string, endDateStr: string | null): string {
  if (!endDateStr || startDateStr === endDateStr) {
    return new Intl.DateTimeFormat("id-ID", {
      ...
      month: "long",
      year: "numeric",
    }).format(parseLocalDate(startDateStr));
  }
...
export function VerticalTimeline({ items, scope }: { items: SharedTimelineItem[]; scope: string }) {
  if (items.length === 0) {
    return (
      <div
        role="tabpanel"
  ...
  return (
    <div
      ...
      id={`panel-${scope}`}
      aria-labelledby={`tab-${scope}`}
      className="relative mx-auto max-w-[850px] px-4 py-8 lg:py-12"
    >
      {/* Central Line */}
